...
Shark.sample.ids <- intersect(rownames(SharkMeta), rownames(tdata3))
Shark.sub <- tdata3[Shark.sample.ids,]
Sharkmeta.sub <- SharkMeta[Shark.sample.ids,]
# double-check that the mapping file and otu table
# had overlapping samples
...
adonis(SharkDataSansWater~SharkMeta$Location)
...
mean(richness)
sd(richness)
boxplot(richness~SharkMeta$Species, col=c("gray", "red", "green", "blue", "cyan"))
boxplot(richness~SharkMeta$Location, col=c("gray", "red", "green", "blue", "cyan"))
...
# simper
sink("simper.SharkWater.csv")
SharkWatersimp<-simper(Shark.phy, AllMeta$SampleType, permutations = 499)
